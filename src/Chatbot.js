...
function Chatbot() {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/metadata_file'); // Replace with your API endpoint
        
        const files = Object.keys(response.data).map(key => ({
          name: response.data[key].name
        }));
        console.log(files);
        setFiles(files); // Assuming response.data is an array of file names
      } catch (error) {
...
const handleSubmit = async (e) => {
  ...
  const handleFileSelection = (fileName) => {
    setSelectedFile(fileName); // Store the selected file name in the state
    setLoading(true);
    fetch('http://127.0.0.1:5000/load_file', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ fileName }), // Pass the selected file name directly
    })
      .then(response => response.json())
      .then(data => {
        setLoading(false);
        console.log('File selected:', data.message);
        // Display message or handle the response
      })
      .catch(error => {
